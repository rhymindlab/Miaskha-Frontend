...
export default function JewelleryInfo({
  ...
  let subtotal = afterDiscountSubTotal;
    ...
    <div className="w-full">
      <div className="lg:sticky lg:top-24 space-y-8">
        ...
        <div className="rounded-3xl border border-neutral-200 bg-white p-8 shadow-sm">
          {Number(product.salePrice) > subtotal && (
            <div className="flex items-center gap-4 mb-4">
              <span className="line-through text-neutral-400 text-xl">
                ₹ {Number(product.salePrice).toLocaleString("en-IN")}
              </span>

              {discount > 0 && (<span className="rounded-full bg-emerald-100 text-emerald-700 px-3 py-1 text-sm font-semibold">
                {discount}% OFF
              </span>)}
